...
class QLearningAgent:
    ...
    def __init__(self, env, bins, alpha=0.02, gamma=0.99,
        ...
        if len(bins) != self.env.observation_space.shape[0]:
            print("Number of bins should be equal to the number of dimensions of the state space")
            sys.exit()
        self.bins = bins
    ...
    def create_state_grid(self):
        state_grid = []
        for bin, i in zip(self.bins, range(self.env.observation_space.shape[0])):
            state_grid.append(np.linspace(self.env.observation_space.low[i], self.env.observation_space.high[i], bin))
